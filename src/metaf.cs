...
#endif
	class CmdLnParms {
		public static string version = "METa Alternate Format (metaf), v.0.7.3.0     GPLv3 Copyright (C) 2021     J. Edwards";
...
Running metaf requires that .NET Core (5.0) be installed. Get it here: https://dotnet.microsoft.com/download
The three files (exe, dll, json) are also required in order to run. The metaf.xml (or metaf_dark.xml) file defines custom
colors for easier editing in Notepad++. Finally, you may need to authorize the file to run since I haven't formally published
it with a digital signature. (The only thing the program does is read a text-based file you tell it to, and write out another
one in a different format.)

I have endeavored to lower the learning curve as much as I can. metaf can literally translate between .met/.nav and .af files
by simply dragging either file type onto its metaf.exe icon. (It auto-detects the file type and translates it to the other
...
you're actively developing a meta and repeatedly translating it to load in-game. (Read on...)
...
new files every time. (Note that 'unusual' characters in paths and file names (e.g., parentheses) may disrupt the batch
file's successful execution.)
	...
	~~ ACTION operations (DO:)		
		  ...
		  ClearWatchdog - Input: none. Clears watchdog for this state.
			 ...
			 CreateView - Input: s View, s XML. Creates a view View, with characteristics defined in XML. Note: the XML
						  ...
						  on the same line as its CreateView operation.) See CreateView in Section 7 for more on external
						  XML files.
