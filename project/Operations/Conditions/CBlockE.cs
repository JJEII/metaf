...
namespace Metaf
{
    public class CBlockE : Condition
    {
        ...
        override public void ImportFromMet(ref FileLines f)
        {
            ...
            try { _block = Int32.Parse(f.line[f.L++]); }
        ...
        override public void ImportFromMetAF(ref FileLines f)
        {
            ...
            try { _block = Int32.Parse(match.Groups["h"].Value, System.Globalization.NumberStyles.HexNumber); }
        ...
        {
            f.line.Add($"{new string('\t', depth)}{typeid} {_block:X8}");
        }
    }
}